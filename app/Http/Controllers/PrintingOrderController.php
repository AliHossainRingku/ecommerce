...
class PrintingOrderController extends Controller
{
    public function printingOrder(){
        ...
        return view('frontEnd.printing.printingOrder');
    }

    public function postPrintingOrder(Request $request){
        //dd($request->all());
        $printing = array();

        $printing['project_name'] = $request->project_name;
        $printing['quantity'] = $request->quantity;
        $printing['project_description'] = $request->project_description;
        $printing['company_name'] = $request->company_name;
        $printing['contact_person_name'] = $request->contact_person_name;
        $printing['company_mobile'] = $request->company_mobile;
        $printing['company_email'] = $request->company_email;
        $printing['district'] = $request->district;
        $printing['postal_code'] = $request->postal_code;
        $printing['file_attach'] = $request->file_attach;

        $printing_order_id = DB::table('tbl_printing_order')
           ->insertGetId($printing);

        $image = $request->file_attach;
        if($image){
            $image_name = $printing_order_id;
            $ext = strtolower('png');
            $image_full_name = $image_name.'.'.$ext;
            $upload_path = 'public/printing_uploaded_file/';
            ...
            if($success){
                ...
                DB::table('tbl_printing_order')
                ->where('printing_order_id',$printing_order_id)
                ->update($printing);
        ...
        return redirect()->back()->with('msg','Successfully done! Your Order no:'.$printing_order_id);
